...
export default function Dashboard({ email, supportedTickers, onLogout }) {
  ...
  useEffect(() => {
    ...
    const socket = io("https://stock-g0zg.onrender.com", {
    ...
    socket.on("init", ({ subscriptions = [], prices = {} }) => {
  ...
  return (
    <div className="dashboard">
      ...
      <div className="card summary-card">
        ...
        {subscriptions.length === 0 ? (
          <p className="muted empty-state">No subscribed stocks yet.</p>
        ) : (
          <div className="summary-grid">
            <div>
              <div className="total-value">${totalValue.toFixed(2)}</div>
              <div className="stats-grid">
                <div>Subscribed: {subscriptions.length}</div>
                <div>Avg: ${avgPrice.toFixed(2)}</div>
                <div>Highest: {topStock?.ticker}</div>
                <div>Lowest: {lowestStock?.ticker}</div>
              </div>
            </div>
            <AllocationChart subscriptions={subscriptions} prices={prices} />
          </div>
      ...
      <div className="dashboard-grid">
        ...
        <div className="card">
          ...
          <table className="table">
            <thead>
              <tr><th>Ticker</th><th>Status</th><th>Action</th></tr>
            </thead>
            <tbody>
              {supportedTickers.map(ticker => (
                <tr key={ticker}>
                  <td>{ticker}</td>
                  <td>{isSubscribed(ticker) ? "Active" : "Inactive"}</td>
                  <td>
                    <button onClick={() => handleToggleSubscription(ticker)}>
                      {isSubscribed(ticker) ? "Unsubscribe" : "Subscribe"}
                    </button>
                  </td>
                </tr>
        ...
        <div className="card">
          ...
          {subscriptions.length === 0 ? (
            ...
            <table className="table">
              ...
              <tbody>
                {subscriptions.map(t => (
                  <PriceRow key={t} ticker={t} price={prices[t]} />
                ))}
              </tbody>
            </table>
          )}
...
function PriceRow({ ticker, price }) {
  ...
  return (
    <tr>
      <td>{ticker}</td>
      <td className={dir}>{price ? `$${price.toFixed(2)}` : "..."}</td>
    </tr>
  );
}
    ...
    <ResponsiveContainer width={200} height={200}>
      <RePieChart>
        <Pie data={data} dataKey="value" innerRadius={50} outerRadius={80}>
          {data.map((_, i) => (
            <Cell key={i} fill={["#6366f1","#0ea5e9","#10b981","#f43f5e","#8b5cf6"][i % 5]} />
          ))}
        </Pie>
        <Tooltip />
      </RePieChart>
